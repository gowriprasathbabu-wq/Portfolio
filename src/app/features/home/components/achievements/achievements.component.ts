import {
  Component, inject, signal, ChangeDetectionStrategy, PLATFORM_ID, AfterViewInit,
} from '@angular/core';
...
@Component({
        ...
        <div class="container">
          ...
          <div class="highlights-grid">
            @for (hl of highlights; track hl.title; let i = $index) {
              <div class="highlight-card reveal glass" [style.transition-delay]="(i * 0.1 + 0.4) + 's'">
  ...
  styles: [`
    ...
    .ach-inner { padding:80px 0; }

    .ach-header { text-align:center;margin-bottom:64px; }
    .eyebrow {
    ...
    .ach-header h2 { font-size:clamp(1.75rem,4vw,2.75rem);font-weight:700;margin:0; }
    .gradient-text {
    ...
    .counter-card {
      ...
      &:hover {
        ...
        box-shadow:0 12px 48px rgba(0,0,0,.5),0 0 40px rgba(255,107,0,.1);
      }
    }
    ...
    .counter-label {
      ...
      color:#A0A0A0;margin-bottom:6px;
    }
    .counter-desc { font-size:.75rem;color:#505050; }
    ...
    .highlight-card {
      ...
      &:hover{border-color:rgba(255,107,0,.2);box-shadow:0 4px 24px rgba(0,0,0,.3);}
    }
    .hl-icon {
      font-size:1.5rem;
    ...
    .hl-content h4 {
      ...
      font-weight:700;color:#C0C0C0;margin:0 0 6px;
    }
    .hl-content p { font-size:.8125rem;color:#606060;margin:0;line-height:1.6; }
...
export class AchievementsComponent implements AfterViewInit {
  ...
  readonly highlights = [
    { icon: '🌍', title: 'PROCON Global Launch', desc: 'Successfully launched enterprise procurement platform across 10+ countries with onsite deployment in Nigeria.' },
    { icon: '🚀', title: 'Enterprise Leadership', desc: 'Led architecture, development, and deployment of 5+ major enterprise products end-to-end.' },
    { icon: '🔒', title: 'Security-First Engineering', desc: 'Implemented Keycloak SSO, OAuth2/JWT authentication for enterprise-grade security across all products.' },
    { icon: '⚡', title: 'AI-Powered Innovation', desc: 'Pioneered integration of Microsoft Copilot in ProcureSprint for next-gen agentic procurement workflows.' },
  ];
  ...
  ngAfterViewInit(): void {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach(e => {
          if (e.isIntersecting) {
            document.querySelectorAll('#achievements .reveal').forEach(el => el.classList.add('revealed'));
            this.startCounters();
            observer.disconnect();
          }
        });
  ...
  private startCounters(): void {
    ...
    const timer = setInterval(() => {
      ...
      const values = numerics.map((target, i) => {
        ...
        if (target >= 1000) return `${(current / 1000).toFixed(current < 1000 ? 1 : 0)}${suffixes[i]}`;
