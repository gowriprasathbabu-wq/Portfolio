...
@Component({
    ...
    <section id="skills" class="section skills-section" aria-labelledby="skills-heading">
      <div class="container">
        ...
        <div class="section-header reveal">
          ...
          <p>A comprehensive skill set spanning frontend, backend, cloud, and security — built through enterprise product development.</p>
        </div>
  ...
  styles: [`
    ...
    .skill-tab {
      ...
      color: #707070; font-size: .875rem; font-weight: 500;
      cursor: pointer; transition: all .2s ease;
      font-family: 'Inter', sans-serif;

      &:hover { color: #C0C0C0; border-color: rgba(255,255,255,.15); }
      &.active {
    ...
    /* ---- Skills grid ---- */
    .skills-panel { animation: fadeIn .3s ease; }
    ...
    .skill-level-label {
      ...
      letter-spacing: .06em; color: #505050;
    }
...
export class SkillsComponent implements AfterViewInit {
  ...
  readonly allTechs = [
    { name: 'Angular 20', color: '#FF6B00' }, { name: 'TypeScript', color: '#3B82F6' },
    { name: 'React', color: '#61DAFB' }, { name: 'Ionic', color: '#3880FF' },
    { name: 'ASP.NET Core', color: '#512BD4' }, { name: 'C#', color: '#512BD4' },
    { name: 'MSSQL', color: '#CC2927' }, { name: 'Entity Framework', color: '#512BD4' },
    { name: 'AWS EC2', color: '#FF9900' }, { name: 'AWS ECS', color: '#FF9900' },
    { name: 'AWS EKS', color: '#FF9900' }, { name: 'Docker', color: '#2496ED' },
    { name: 'Keycloak', color: '#A855F7' }, { name: 'OAuth2', color: '#A855F7' },
    { name: 'JWT', color: '#FB7185' }, { name: 'SonarQube', color: '#4E9BCD' },
    { name: 'GitHub Actions', color: '#22C55E' }, { name: 'RxJS', color: '#B7178C' },
    { name: 'SignalR', color: '#512BD4' }, { name: 'REST APIs', color: '#10B981' },
    { name: 'SCSS', color: '#CC6699' }, { name: 'HTML5', color: '#E34F26' },
    { name: 'SDLC', color: '#6B7280' }, { name: 'Agile', color: '#0EA5E9' },
  ];
  ...
  ngAfterViewInit(): void {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach(e => {
          if (e.isIntersecting) {
            document.querySelectorAll('#skills .reveal').forEach(el => el.classList.add('revealed'));
            observer.disconnect();
          }
        });
      },
