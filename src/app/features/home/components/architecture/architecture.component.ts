    ...
    <section id="architecture" class="section arch-section" aria-labelledby="arch-heading">
      <div class="container">
        <div class="section-header reveal">
          ...
          <p>
            A robust, layered architecture powering enterprise applications — from pixel to cloud.
          </p>
        </div>
  ...
  styles: [
    `
      ...
      .arch-node {
        ...
        &::before {
          ...
          background: linear-gradient(
            135deg,
            rgba(from var(--node-color, #ff6b00) r g b / 0.04),
            transparent
          );
        }
      ...
      .connector-line {
        width: 2px;
        height: 20px;
        background: linear-gradient(180deg, rgba(255, 107, 0, 0.3), var(--color, #3b82f6));
        border-radius: 1px;
      }
      .connector-arrow {
        color: #505050;
  ...
  readonly principles = [
    {
      ...
      desc: 'Separation of concerns with feature-based modules and dependency inversion.',
    },
    {
      icon: '🔒',
      title: 'Security by Design',
      desc: 'Keycloak SSO, JWT, OAuth2 baked in from the ground up.',
    },
    {
      icon: '📈',
      title: 'Horizontal Scalability',
      desc: 'ECS/EKS container orchestration with load balancing for auto-scale.',
    },
    {
      icon: '✅',
      title: 'Quality Gates',
      desc: 'SonarQube analysis enforced in CI/CD pipeline before every deploy.',
    },
  ];
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((e) => {
          if (e.isIntersecting) {
            document
              .querySelectorAll('#architecture .reveal')
